...
describe("PokerToken", () => {
    ...
    describe("Initial State", async () => {
        ...
        it("Should have 'name', 'symbol' and decimals setup", async () => {
            console.log(await deployments.get("PokerToken"));
    ...
    //         await expect(
    //             pokerTokenContract.connect(holder1).mint(await holder2.getAddress(), DEFAULT_AMOUNT)
    //         ).to.be.reverted;
    ...
    //         await expect(pokerTokenContract.connect(spender).transferFrom(holder1.address, spender.address, 2 * ALLOWED_AMOUNT))
    //             .to.be.reverted;
    //     });
    // });
});
