...
import { StatusCode, EngineBetType, EngineStep } from "./lib/Engine";
import { PokerEngine } from "./lib/PokerEngine"
import { expect } from 'chai';

describe('A complete game', () => {
  it('should run without errors - The happy path', async () => {
    const alice = new PokerEngine(0);
    expect(await alice.init(BigNumber.from(200),  BigNumber.from(300), BigNumber.from(10), true)).to.equal(StatusCode.SUCCESS);
    const bob = new PokerEngine(1);
    expect(await bob.init(BigNumber.from(200),  BigNumber.from(300), BigNumber.from(10), true)).to.equal(StatusCode.SUCCESS);

    // Handshake
    const r0 = await alice.create_handshake();
    expect(r0.status).to.equal(StatusCode.SUCCESS);
